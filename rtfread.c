...
unsigned short int rtf_to_unicode(int code, const short int *current_charset);
RTFTypes getCommandType(char *name);
signed int getCharCode(FILE *f);
void rtfSetCharset(const short int **charset_ptr,unsigned int codepage, enum CHARSETS *source_csname);
...
void add_to_buffer(int *bufptr,unsigned short int c, FILE *out, unsigned short int *buffer, char outputbuffer[LINE_BUF_SIZE], int *bufpos) {
	buffer[++(*bufptr)]=c;
...
void end_paragraph(int *bufptr, FILE *out, unsigned short int *buffer, char outputbuffer[LINE_BUF_SIZE], int *bufpos) {
    add_to_buffer(bufptr,0x000a, out, buffer, outputbuffer, bufpos);
    add_to_buffer(bufptr,0, out, buffer, outputbuffer, bufpos);
    output_paragraph(buffer, out, outputbuffer, bufpos);
    *bufptr=-1;
}				   
...
int parse_rtf(FILE *f, FILE *out) {
	...
	int bufptr=-1;
    enum CHARSETS source_csname = CP_1251;
    unsigned short int *buffer = (unsigned short int*) malloc(PARAGRAPH_BUFFER * sizeof(unsigned short int));
    const short int *current_charset = source_charset;
...
/* 				com.name, com.numarg); */
				if (com.numarg == '*' && data_skip_mode == 0) {
					...
					end_paragraph(&bufptr, out, buffer, outputbuffer, &bufpos);
				} else if (com.numarg == '~') {
					add_to_buffer(&bufptr,0xA0, out, buffer, outputbuffer, &bufpos);/* NO-BREAK SPACE */
				} else if (com.numarg == '-') {
					add_to_buffer(&bufptr,0xAD, out, buffer, outputbuffer, &bufpos);/* Optional hyphen */
				}	

				   break;
			case RTF_EMDASH:
                add_to_buffer(&bufptr,0x2014, out, buffer, outputbuffer, &bufpos);/* EM DASH*/
				   break;
			case RTF_ENDASH: 
                add_to_buffer(&bufptr,0x2013, out, buffer, outputbuffer, &bufpos);break;
			case RTF_BULLET: 
                add_to_buffer(&bufptr,0x2022, out, buffer, outputbuffer, &bufpos);break;
			case RTF_LQUOTE: add_to_buffer(&bufptr,0x2018, out, buffer, outputbuffer, &bufpos);break;
			case RTF_RQUOTE: add_to_buffer(&bufptr,0x2019, out, buffer, outputbuffer, &bufpos);break;
			case RTF_LDBLQUOTE: add_to_buffer(&bufptr,0x201C, out, buffer, outputbuffer, &bufpos);break;
			case RTF_RDBLQUOTE: add_to_buffer(&bufptr,0x201D, out, buffer, outputbuffer, &bufpos);break;
			case RTF_ZWNONJOINER: add_to_buffer(&bufptr,0xfeff, out, buffer, outputbuffer, &bufpos);break;
			case RTF_EMSPACE:
			case RTF_ENSPACE:
                add_to_buffer(&bufptr,' ', out, buffer, outputbuffer, &bufpos);break;
			case RTF_CHAR:
/*  				fprintf(stderr, "RTF char %d\n", com.numarg); */
				if (data_skip_mode == 0) {
				 	add_to_buffer(&bufptr,rtf_to_unicode(com.numarg, current_charset), out, buffer, outputbuffer, &bufpos);
				}	
				break;
			case RTF_UC:
				...
				break;
			case RTF_TAB:
				add_to_buffer(&bufptr,0x0009, out, buffer, outputbuffer, &bufpos);
				break;
...
/*  				fprintf(stderr, "Unicode char %d\n", com.numarg);  */
				if (data_skip_mode == 0)
					add_to_buffer(&bufptr,com.numarg, out, buffer, outputbuffer, &bufpos);
...
/* 				fprintf(stderr, "Selected lang = %d\n",com.numarg); */
				break;
			case RTF_CODEPAGE:
				rtfSetCharset(&current_charset,com.numarg, &source_csname);
			default:
...
/*  						com.name, com.numarg);  */
		...
		case '{':
			...
			if (para_mode)
				add_to_buffer(&bufptr,0x20, out, buffer, outputbuffer, &bufpos);
		...
		default:
			if (data_skip_mode == 0)
				if (c != '\n' && c != '\r')
					add_to_buffer(&bufptr,rtf_to_unicode(c, current_charset), out, buffer, outputbuffer, &bufpos);
		}
	}
	if (bufptr>=0) {
		add_to_buffer(&bufptr,'\n', out, buffer, outputbuffer, &bufpos);
		add_to_buffer(&bufptr,0, out, buffer, outputbuffer, &bufpos);
		output_paragraph(buffer, out, outputbuffer, &bufpos);
	}	
	free(groups);
...
void rtfSetCharset(const short int **charset_ptr,unsigned int codepage, enum CHARSETS *source_csname)
{
    ...
    if (charset_name != C_NULL) *source_csname = charset_name;
	*charset_ptr = source_charsets[*source_csname];
}
